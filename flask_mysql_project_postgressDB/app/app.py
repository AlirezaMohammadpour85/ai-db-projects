...
def get_db_connection():
    ...
    return psycopg2.connect(
        ...
        dbname=os.getenv("DB_NAME", "domotz"),  # Changed from `database` to `dbname`
    )


def create_table_from_csv(cursor, table_name, header):
    """
    Creates a table in the PostgreSQL database with columns derived from the CSV header.
    Uses TEXT data type to avoid PostgreSQL row size limit.
    """
    drop_table_sql = f'DROP TABLE IF EXISTS "{table_name}";'
    cursor.execute(drop_table_sql)

    # Use TEXT instead of VARCHAR to avoid row size error
...
def index():
    ...
    try:
        ...
        if "cursor" in locals() and cursor:
            cursor.close()
        # Check if the connection is open (PostgreSQL)
        if "db" in locals() and db.closed == 0:
            db.close()
